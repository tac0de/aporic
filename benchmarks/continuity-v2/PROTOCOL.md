# Continuity/resume benchmark v2 — preregistration

Version 1 stopped after C01 treatment and baseline and C02 baseline. The
read-only CLI sandbox prevented the Aporic SQLite server from opening its
database (`SqliteFailure CannotOpen`), so v1 is an invalid instrument check,
not a product comparison. Its partial raw runs remain at
`/private/tmp/aporic-continuity-v1/runs`. A separate preflight verified that
the same C01 treatment helper opens the database under `workspace-write`.
Version 2 uses fresh isolated workspaces, keeps all eight case facts and gold
answers unchanged, and is the first interpretable batch. The known v1 C01
answer must not be pooled with v2.
...
- Use `codex exec --ephemeral --ignore-user-config --skip-git-repo-check
  --sandbox workspace-write --json --output-schema`, with the CLI default model
