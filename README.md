...
Grant consumption records preflight admission, not successful execution or filesystem effect. `PostToolUse` records an `unknown` action outcome because Codex tool responses are tool-specific; a later evidence-backed verification must establish success or failure. `SessionEnd` publishes a transcript-independent checkpoint from recorded state, and the next distinct session can claim one open checkpoint exactly once. Exact input storage can persist sensitive tool arguments in the event log; do not issue a grant containing secrets.
