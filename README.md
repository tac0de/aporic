...
- An agent opening a blocking knowledge gap must either request specific material or state its collection method, source-selection criteria, and intended use; resolving that gap requires same-scope non-inference evidence.
...
The packaged `UserPromptSubmit` hook applies intent-fidelity contract v1 to every user turn in a bound project. The contract asks the model to preserve explicit actors, targets, exclusions, negation, conditions, sequence, uncertainty, authorization boundaries, and exact technical strings; classify material meaning as explicit, inferred, or unknown; and clarify consequential ambiguity rather than guessing. For a consequential unknown, it must plainly disclose the gap, request one specific user-held material only when necessary, or state its own collection method, source-selection criteria, and intended decision use. Its context is deterministically limited to 1,200 UTF-8 bytes. The adapter validates project discovery but does not interpret, persist, hash, echo, or block the prompt. Raw hook input is limited to 1 MiB; an oversized payload skips the advisory without blocking the conversation. The advisory does not authenticate authority, grant permission, or replace `PreToolUse`.
