...
use aporic::{
    ...
    EvidenceKind, SCHEMA_VERSION, VerificationResult, commit, initialize, load, migrate_v2_to_v3,
};
use serde_json::json;
...
#[test]
fn schema_two_logs_migrate_to_schema_three_without_mutating_source() {
    ...
    let outcome = migrate_v2_to_v3(&source, &destination).unwrap();
    assert_eq!(outcome.from_schema, 2);
    assert_eq!(outcome.to_schema, 3);
