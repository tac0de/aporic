...
fn repair_obligation_remains_visible_until_a_verified_repair_task_completes() {
    ...
    assert_eq!(restarted.stats().unwrap().schema_version, 19);
    ...
    let export = restarted
        ...
        .unwrap();
    assert_eq!(export.format_version, 15);
...
fn upgrades_schema_18_to_accountability_schema() {
    ...
    let connection = rusqlite::Connection::open(&database).unwrap();
    connection
        .execute_batch("DROP TABLE accountability_cases; PRAGMA user_version = 18;")
        .unwrap();
    drop(connection);
